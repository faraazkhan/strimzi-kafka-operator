...
public class Exec {
    ...
    public static ExecResult exec(String input, List<String> command, int timeout, boolean logToOutput) {
        try {
            ...
            int ret = executor.execute(input, command, timeout);
            synchronized (LOCK) {
                if (logToOutput) {
            ...
            ex.printStackTrace();
            return new ExecResult(1, null, null);
        }
    }
     ...
     * @throws ExecutionException
     */
    private int execute(String input, List<String> commands, int timeout) throws IOException, InterruptedException, ExecutionException {
